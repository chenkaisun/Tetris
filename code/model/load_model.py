...
from transformers import EncoderDecoderModel, AutoModelForCausalLM, BertGenerationEncoder, BertGenerationDecoder, EncoderDecoderConfig, AutoConfig, \
    BartForConditionalGeneration, T5ForConditionalGeneration, GPT2LMHeadModel, AutoModel
...
from model.script_learner import ScriptLearner
# import wandb
def get_model(args, tokenizer, **kwargs):
    ...
    if args.model_name == "bsl_bert":
    ...
    elif args.do_cl or args.model_name == "sl":
        if args.task_mode in ["gen",'prt'] :
            if "bart-tiny" in args.plm:
                model=ScriptLearner.from_pretrained(args.plm)
            else:
                model=ScriptLearner.from_pretrained(args.plm, revision=revision_id)
            model.add_modules(args)
            print("revision")

    elif args.model_name == "bsl_bart":
        if args.task_mode in ["gen",'prt'] :
            if "bart-tiny" in args.plm:
        ...
        # decoder default plm
    elif args.model_name == "bsl_t5":
        model=T5ForConditionalGeneration.from_pretrained(args.plm)
        # decoder default plm
    elif args.model_name == "bsl_gpt2":
        model = GPT2LMHeadModel.from_pretrained(args.plm)
    elif args.model_name == "see":
        print("\nLoad Model See")
        # encoder = EventReasoningEncoder.from_pretrained(args.plm) #, bos_token_id=tokenizer.cls_token_id, eos_token_id=tokenizer.sep_token_id
        # encoder = BertGenerationEncoder.from_pretrained(args.plm) #, bos_token_id=tokenizer.cls_token_id, eos_token_id=tokenizer.sep_token_id
        if args.task_mode in ["gen",'prt'] :
            print("SEEBartForConditionalGeneration",)

            model=SEEBartForConditionalGeneration.from_pretrained(args.plm, revision=revision_id) #, args=args, pretrained_concept_emb=kwargs['pretrained_concept_emb']
            print("revision",revision_id)
            # model.model.encoder.add_modules(args, **kwargs)
            if args.freeze_plm:
                for param in model.parameters():
                    param.requires_grad = False
            model.model.add_modules(args, **kwargs)
            # print("e0")
            # print("e1")
            # if "decattn" in args.components:
            #     model.model.decoder.layers[args.decattn_layer_idx].add_modules(args)

        elif args.task_mode == "clf":
            model = FET(args,tokenizer)
            model.plm.add_modules(args, **kwargs)

    elif args.model_name == "retriever":
    ...
    if args.task_mode in ["gen",'prt'] :
        ...
        # if not args.do_sample: model.config.do_sample = False
