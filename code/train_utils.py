...
from transformers import BartModel, BartConfig, T5TokenizerFast, T5Tokenizer, GPT2Tokenizer
...
def setup_common(args, tokenizer, concept_index_mapping=None):
    ...
    if args.load_model:
        ...
        if "/" not in args.load_model_path and ".pt" not in args.load_model_path:
            model_data=wandb.restore(f"{args.load_model_path}.pt", run_path=f"NS-code/{args.load_model_path}")
...
def get_tokenizer(plm, save_dir="tokenizer/", slow_connection=False):
    ...
    if "t5-" in plm: # in ["patrickvonplaten/t5-tiny-random","t5-small","t5-base","t5-large"]
        ...
        tokenizer = GPT2Tokenizer.from_pretrained('gpt2', bos_token='<|startoftext|>', cls_token='<|startoftext|>', eos_token='<|endoftext|>', sep_token='.',
                                                  pad_token='<|pad|>')
    return tokenizer
